...
##                        if is_imply:
##                            if i+2<len(formula):
##                                while formula[i+2+offset]=="imply":
##                                    offset+=2
##                                    if i+2+offset>=len(formula):
##                                        break
...
def remove_not(formula:list)->list:
    if type(formula)!=list or len(formula)==1:
        return formula
    if len(formula)==2:
        if type(formula[1])!=list or len(formula[1])==1:
            return formula
        if len(formula[1])==2:
...
def parse(formula:list)->list:
    ...
    while i<len(formula):
        ...
        if token=="(":
            ...
            new.append(token)
        i+=1
    if len(new)==1 and type(new[0])==list:
        return new[0]
    return new
    ...
    for elm in formula:
        if type(elm)==list:
            ...
            for val in new_var:
                if val not in var:
                    var.append(val)
        elif elm not in operators and elm not in var:
            var.append(elm)
    return sorted(var)
...
def oor(A:list,B:list):
    ...
    if len(A)==3 and len(B)==3:
        ...
        return to_cnf([[[[A[0],"or",B[0]]],"and",[[A[0],"or",B[2]]]],"and",[[[A[2],"or",B[0]]],"and",[[A[2],"or",B[2]]]]])
    return [A,"and",B]
...
def dimacs(cnf:list,var:list):
    string=f"p cnf {len(var)} {(len(cnf)+1)//2}\n"
    for clause in cnf:
        if clause=="and":
            continue
        n_string=""
                ...
                for unit2 in units:
                    ...
                    for phi in unit:
                        for psi in unit2:
                            if phi==-1*psi:
                                add_node(unit,unit2,"{}")
                                print_html(f"<tr><td>{len(new_cnf) + 1}. </td>" + "<td>{}</td>" + f' <td style="white-space:nowrap">({new_cnf.index(unit) + 1})({new_cnf.index(unit2) + 1})</td><tr>')
                                return True
        found_unit=False
        found=False
        for unit in units:
            for mult in multi:
                ...
                for phi in unit:
                    if phi*-1 in mult:
                        ...
                        print_html("}</td>")
                        print_html(f' <td style="white-space:nowrap">({new_cnf.index(unit) + 1})({new_cnf.index(mult) + 1})</td></tr>')
                        new_cnf.append(new)
                        if len(new)==1:
                            found_unit=True
                            units.add(new)
            ...
            for B in multi:
                ...
                for phi in A:
                    for psi in B:
                        if phi==-1*psi:
                            found=True
                            new=set([term for term in A if term!=phi]).union(set([term for term in B if term!=psi]))
                            if new in units or new in multi:
                                found=False
                                continue
                            ...
                            print_set(new,table=True)
                            print_html("}</td>")
                            print_html(f' <td style="white-space:nowrap">({new_cnf.index(A) + 1})({new_cnf.index(B) + 1})</td></tr>')
                            new_cnf.append(new)
                            if len(new)==1:
                                found_unit=True
                ...
                for phi in unit:
                    for psi in result:
                        if phi==-1*psi:
                            ...
                            print_html(f"<tr><td>{len(new_cnf) + 1}. </td>" + "<td>{}</td>" + f' <td style="white-space:nowrap">({new_cnf.index(unit) + 1})({new_cnf.index(result) + 1})</td><tr>')
                            return True
        found_unit=False
        found=False
        if result is None:
            ...
            for A in multi:
                ...
                for phi in A:
                    for psi in result:
                        if phi==-1*psi:
                            found=True
                            new=set([term for term in A if term!=phi]).union(set([term for term in result if term!=psi]))
                            if new in units or new in multi:
                                found=False
                                continue
                            ...
                            print_set(new,table=True)
                            print_html("}</td>")
                            print_html(f' <td style="white-space:nowrap">({new_cnf.index(A) + 1})({new_cnf.index(result) + 1})</td></tr>')
                            new_cnf.append(new)
                            if len(new)==1:
                                found_unit=True
                                units.add(frozenset(new))
            ...
            for elm in q:
                if formula==elm:
                    ...
                    if elm==q[0]:
                        first=True
        if red:
            print_html(' <font color="#FF0000"> '+print_recursive(formula[0],formula[1],q=q,color=color,cnf=cnf),formula[1],print_recursive(formula[2],formula[1],right=True,q=q,color=color,cnf=cnf)+" </font> ")
        else:
            print_html(print_recursive(formula[0],formula[1],q=q,color=color,cnf=cnf),operator,print_recursive(formula[2],formula[1],right=True,q=q,color=color,cnf=cnf))
    print_html("\n")
...
class Node:
    ...
    def intersects(self,other):
        if self.y==other.y and other.is_active() and self.x<=other.x+get_pixel_length(other.value,16,"arial.ttf") and self.x+get_pixel_length(self.value,16,"arial.ttf")>=other.x:
            return True
        return False
...
def add_node(s1,s2,val):
    ...
    if s2==None:
        nodes.append(Node(node1.x+25+get_pixel_length(node1.value.replace("not ", "¬ "),16,"arial.ttf"),node1.y,val))
        return nodes[-1]
